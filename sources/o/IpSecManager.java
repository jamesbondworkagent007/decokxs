package o;

import kotlin.jvm.internal.Intrinsics;
import o.AbstractC5105Ex;
import o.C5098Eq;
import org.jetbrains.annotations.NotNull;

/* JADX INFO: loaded from: classes21.dex */
public final class IpSecManager {
    public static final void AEQbTJ(@NotNull InterfaceC5106Ey interfaceC5106Ey, @NotNull WallpaperBackupHelper wallpaperBackupHelper) {
        Intrinsics.checkNotNullParameter(interfaceC5106Ey, "");
        Intrinsics.checkNotNullParameter(wallpaperBackupHelper, "");
        C5101Et c5101Et = new C5101Et(AbstractC5105Ex.StateListAnimator.copydefault, new EX("DefaultEmailOption"));
        AbstractC5105Ex.LoaderManager loaderManager = AbstractC5105Ex.LoaderManager.KWHzl;
        C5101Et c5101Et2 = new C5101Et(loaderManager, new EX("EmailMessage"));
        C5101Et c5101Et3 = new C5101Et(loaderManager, new EX("EmailMessageByLink"));
        C5101Et c5101Et4 = new C5101Et(loaderManager, new EX("EmailSubject"));
        C5101Et c5101Et5 = new C5101Et(loaderManager, new EX("EmailSubjectByLink"));
        C5101Et c5101Et6 = new C5101Et(loaderManager, new EX("SmsMessage"));
        C5098Eq.TaskDescription taskDescription = C5098Eq.OLrzqt;
        C5098Eq.Activity activity = new C5098Eq.Activity();
        activity.EZpvd(c5101Et);
        activity.EZpvd(c5101Et2);
        activity.EZpvd(c5101Et3);
        activity.EZpvd(c5101Et4);
        activity.EZpvd(c5101Et5);
        activity.EZpvd(c5101Et6);
        EB ebEZpvd = interfaceC5106Ey.EZpvd(activity.KWHzl());
        java.lang.String strDjBIcL = wallpaperBackupHelper.djBIcL();
        if (strDjBIcL != null) {
            ebEZpvd.AEQbTJ(c5101Et6, strDjBIcL);
        }
        java.lang.String strAEQbTJ = wallpaperBackupHelper.AEQbTJ();
        if (strAEQbTJ != null) {
            ebEZpvd.AEQbTJ(c5101Et2, strAEQbTJ);
        }
        java.lang.String strEZpvd = wallpaperBackupHelper.EZpvd();
        if (strEZpvd != null) {
            ebEZpvd.AEQbTJ(c5101Et4, strEZpvd);
        }
        java.lang.String strCopydefault = wallpaperBackupHelper.copydefault();
        if (strCopydefault != null) {
            ebEZpvd.AEQbTJ(c5101Et3, strCopydefault);
        }
        java.lang.String strOLrzqt = wallpaperBackupHelper.OLrzqt();
        if (strOLrzqt != null) {
            ebEZpvd.AEQbTJ(c5101Et5, strOLrzqt);
        }
        AnyThread anyThreadKWHzl = wallpaperBackupHelper.KWHzl();
        if (anyThreadKWHzl != null) {
            ebEZpvd.AEQbTJ(c5101Et, anyThreadKWHzl.OLrzqt());
        }
        ebEZpvd.AEQbTJ();
    }
}
