...
public class PKIXNameConstraintValidator {
    ...
    private void intersectEmail(String str, String str2, Set set) {
        if (str.indexOf(64) != -1) {
            ...
            if (str2.indexOf(64) != -1) {
                ...
                return;
            } else if (str2.startsWith(JwtUtilsKt.JWT_DELIMITER)) {
                if (withinDomain(strSubstring, str2)) {
                    set.add(str);
                    return;
                }
    ...
    private boolean isUriConstrained(String str, String str2) {
        ...
        return !str2.startsWith(JwtUtilsKt.JWT_DELIMITER) ? strExtractHostFromURL.equalsIgnoreCase(str2) : withinDomain(strExtractHostFromURL, str2);
    }
    ...
    public int hashCode() {
        int iHashCollection = hashCollection(this.excludedSubtreesDN);
        int iHashCollection2 = hashCollection(this.excludedSubtreesDNS);
        int iHashCollection3 = hashCollection(this.excludedSubtreesEmail);
        int iHashCollection4 = hashCollection(this.excludedSubtreesIP);
        int iHashCollection5 = hashCollection(this.excludedSubtreesURI);
        int iHashCollection6 = hashCollection(this.permittedSubtreesDN);
        int iHashCollection7 = hashCollection(this.permittedSubtreesDNS);
        int iHashCollection8 = hashCollection(this.permittedSubtreesEmail);
        return iHashCollection + iHashCollection2 + iHashCollection3 + iHashCollection4 + iHashCollection5 + iHashCollection6 + iHashCollection7 + iHashCollection8 + hashCollection(this.permittedSubtreesIP) + hashCollection(this.permittedSubtreesURI);
    }
    ...
    private String stringifyIP(byte[] bArr) {
        ...
        for (int i = 0; i < bArr.length / 2; i++) {
            str = str + Integer.toString(bArr[i] & UnsignedBytes.MAX_VALUE) + JwtUtilsKt.JWT_DELIMITER;
        }
        String str2 = str.substring(0, str.length() - 1) + "/";
        for (int length = bArr.length / 2; length < bArr.length; length++) {
            str2 = str2 + Integer.toString(bArr[length] & UnsignedBytes.MAX_VALUE) + JwtUtilsKt.JWT_DELIMITER;
        }
