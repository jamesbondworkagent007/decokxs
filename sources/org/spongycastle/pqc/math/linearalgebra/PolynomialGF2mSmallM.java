...
public class PolynomialGF2mSmallM {
    ...
    private int[][] div(int[] iArr, int[] iArr2) {
        ...
        int iComputeDegree2 = computeDegree(iArr);
        if (iComputeDegree == -1) {
            throw new ArithmeticException("Division by zero.");
        }
        int[][] iArr3 = {new int[1], new int[iComputeDegree2 + 1]};
    ...
    private boolean isIrreducible(int[] iArr) {
        ...
        for (int i = 0; i < (iComputeDegree >> 1); i++) {
    ...
    public PolynomialGF2mSmallM[] modPolynomialToFracton(PolynomialGF2mSmallM polynomialGF2mSmallM) {
        ...
        int[] iArr = iArrNormalForm;
        int[] iArrMod = mod(this.coefficients, polynomialGF2mSmallM.coefficients);
        int[] iArr2 = {0};
        int[] iArr3 = {1};
        while (computeDegree(iArrMod) > (i >> 1)) {
            int[][] iArrDiv = div(iArr, iArrMod);
            int[] iArr4 = iArrDiv[1];
            int[] iArrAdd = add(iArr2, modMultiply(iArrDiv[0], iArr3, polynomialGF2mSmallM.coefficients));
            iArr2 = iArr3;
            iArr3 = iArrAdd;
            iArr = iArrMod;
            iArrMod = iArr4;
