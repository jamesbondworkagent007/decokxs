...
public class CamelliaLightEngine implements BlockCipher {
    ...
    private int sbox2(int i) {
        return lRot8(SBOX1[i], 1) & UnsignedBytes.MAX_VALUE;
    }

    private int sbox3(int i) {
        return lRot8(SBOX1[i], 7) & UnsignedBytes.MAX_VALUE;
    }

    private int sbox4(int i) {
        return SBOX1[lRot8((byte) i, 1) & UnsignedBytes.MAX_VALUE] & UnsignedBytes.MAX_VALUE;
    }

    private void camelliaF2(int[] iArr, int[] iArr2, int i) {
        int i2 = iArr[0] ^ iArr2[i];
        int iSbox4 = sbox4(i2 & 255);
        int iSbox3 = sbox3((i2 >>> 8) & 255);
        int iSbox2 = sbox2((i2 >>> 16) & 255);
        byte[] bArr = SBOX1;
        byte b = bArr[(i2 >>> 24) & 255];
        int i3 = iArr[1] ^ iArr2[i + 1];
        int iLeftRotate = leftRotate((sbox2((i3 >>> 24) & 255) << 24) | (bArr[i3 & 255] & UnsignedBytes.MAX_VALUE) | (sbox4((i3 >>> 8) & 255) << 8) | (sbox3((i3 >>> 16) & 255) << 16), 8);
        int i4 = (((b & UnsignedBytes.MAX_VALUE) << 24) | ((iSbox4 | (iSbox3 << 8)) | (iSbox2 << 16))) ^ iLeftRotate;
        int iLeftRotate2 = leftRotate(iLeftRotate, 8) ^ i4;
        int iRightRotate = rightRotate(i4, 8) ^ iLeftRotate2;
        iArr[2] = (leftRotate(iLeftRotate2, 16) ^ iRightRotate) ^ iArr[2];
        iArr[3] = leftRotate(iRightRotate, 8) ^ iArr[3];
        int i5 = iArr[2] ^ iArr2[i + 2];
        int iSbox42 = sbox4(i5 & 255);
        int iSbox32 = sbox3((i5 >>> 8) & 255);
        int iSbox22 = sbox2((i5 >>> 16) & 255);
        byte b2 = bArr[(i5 >>> 24) & 255];
        int i6 = iArr2[i + 3] ^ iArr[3];
        int iLeftRotate3 = leftRotate((sbox2((i6 >>> 24) & 255) << 24) | (bArr[i6 & 255] & UnsignedBytes.MAX_VALUE) | (sbox4((i6 >>> 8) & 255) << 8) | (sbox3((i6 >>> 16) & 255) << 16), 8);
        int i7 = (((b2 & UnsignedBytes.MAX_VALUE) << 24) | (((iSbox32 << 8) | iSbox42) | (iSbox22 << 16))) ^ iLeftRotate3;
        int iLeftRotate4 = leftRotate(iLeftRotate3, 8) ^ i7;
        int iRightRotate2 = rightRotate(i7, 8) ^ iLeftRotate4;
