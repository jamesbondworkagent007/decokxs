...
public class GammaEvaluator {
    ...
    public static int evaluate(float f, int i, int i2) {
        ...
        return (Math.round((f2 + ((f3 - f2) * f)) * 255.0f) << 24) | (Math.round(OECF_sRGB(fEOCF_sRGB + ((fEOCF_sRGB4 - fEOCF_sRGB) * f)) * 255.0f) << 16) | (Math.round(OECF_sRGB(fEOCF_sRGB2 + ((fEOCF_sRGB5 - fEOCF_sRGB2) * f)) * 255.0f) << 8) | Math.round(OECF_sRGB(fEOCF_sRGB3 + ((fEOCF_sRGB6 - fEOCF_sRGB3) * f)) * 255.0f);
    }
}
